...
namespace toxi
{
	namespace math
	{
		...
		ScaleMap::ScaleMap(const ScaleMap& copyFrom)
		{
			...
			this->in = toxi::util::datatypes::DoubleRange( copyFrom.in );
			this->out = toxi::util::datatypes::DoubleRange( copyFrom.out );
		}
		...
		double ScaleMap::getClippedValueFor( const double & val ) const
		{
			double cl = ( val - in.getMin() ) / this->interval; 
			//std::cout << "cl: " << cl << " interval: " << interval << std::endl;
			...
			//std::cout << "out.min: " << out.getMin() << " out.max: " << out.getMax() << " t: " << t << std::endl;
		...
		double ScaleMap::getMappedValueFor( const double & val )
		{
